...
| logs-search（ログを検索） | できた | 1.06% |
| capture（撮影） | できた | 1.29% |
| log（カレンダー） | できた | 1.76% |
| settings（設定） | できた | 1.87% |
| map（マップ） | できた | 2.32% |
| all-search（検索） | できた | 3.07% |
| logset（ログの設定） | できた | 4.01% |
| cut（カットの詳細） | できた | 14.91%（下記） |
| day（その日） | 作り直した（映像＋ドラム） | 対象外 |
| review（撮影後の確認） | 作った | 対象外 |

### まだ残っている差

- **cut 14.91%** — ほとんどが映像の見せ方の違い。web は `<video controls>` で
  ブラウザ標準の操作帯（幅いっぱい・高さ 85px ほど）が出て、絵も動く。
  Flutter は押されるまでサムネを敷いて、丸い再生の印を重ねる。
  器の大きさ（16:9 の 358×201）と、その下の並びは一致している。
  ひとことの「消す」も、web は日画面の側にしか置いていないが、こちらは詳細からも
  自分のぶんだけ消せるようにしてある。ここは合わせない。
- **map 2.32%** — 瓦の絵そのものの描き分け。Leaflet の断り書きに入る「Leaflet |」は、
  Leaflet を使っていないので出さない。
  ★ 43.73% → 2.08% になったのは `forceIntegerZoomLevel`。Leaflet の `fitBounds` は
    倍率を整数へ落とす。合わせないと同じ範囲を入れても半段ぶん寄った絵になる。
  ★ 寄る・引くの四角は、時計や電池の帯のぶんだけ下げてある。web は下げていないが、
    端末では帯の下に隠れて押せなくなる。ここは合わせない（+0.24%）。
- **logset 4.01% / all-search 3.07%** — 札や画面の中の行の高さが 10px ぶんずれている。

### 覚えておくこと

- 札（`.panel`）は 16px の padding を持つので、CSS の余白は端から抜けない。
  `CssColumn(..., outer: true)` にしないと、札の丈が最後の余白ぶん足りなくなる
  （logs-add 4.71% → 1.01%）。
- ひとことの行の上下は padding。Block の余白（margin）にすると隣とくっついて
