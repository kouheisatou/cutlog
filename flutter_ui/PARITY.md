...
| logs-add（ログを追加） | できた | 4.71% |
| cut（カットの詳細） | できた | 13.46%（下記） |
| day（その日） | 作り直した（映像＋ドラム） | 対象外 |
| review（撮影後の確認） | 作った | 対象外 |

### まだ残っている差

- **cut 13.46%** — ほとんどが映像の見せ方の違い。web は `<video controls>` で
  ブラウザ標準の操作帯が出て、絵も動く。Flutter は押されるまでサムネを敷いて、
  丸い再生の印を重ねる。器の大きさ（16:9 の 358×201）は一致している。
  ひとことの「消す」も、web は日画面の側にしか置いていないが、こちらは詳細からも
  自分のぶんだけ消せるようにしてある。ここは合わせない。
- **map 2.08%** — 瓦の絵そのものの描き分け。Leaflet の断り書きに入る「Leaflet |」は、
  Leaflet を使っていないので出さない。
  ★ 43.73% → 2.08% になったのは `forceIntegerZoomLevel`。Leaflet の `fitBounds` は
    倍率を整数へ落とす。合わせないと同じ範囲を入れても半段ぶん寄った絵になる。
- **logs-add 4.71% / logset 4.01%** — 札の中の行の高さが 4〜13px ぶんずれている。
